...
function playerregister(msg)
	...
	if (data_str == nil or data_str == '') then
		...
		local  saveData=newPlayer
		-- saveData["matching"]=0
		-- saveData["financeData"]["gold"]=100
		-- saveData["financeData"]["diamond"]=50
		-- saveData["towerData"]={}
		-- saveData["achieveData"]["trophyAmount"]=0
		-- saveData["achieveData"]["rankObject"]={}
		-- saveData["lineupData"]["lineup"]={}
		-- saveData["otherData"]["headPortrait"]=""
		saveData["pid"] = id
		...
		logInfo = tostring(id) .. " register succ"
	else
		logInfo = tostring(id) .. " register unsucc"
	end
	print(logInfo)
	qlog(LogDef.CHANNEL.DEBUG, logInfo)
end
