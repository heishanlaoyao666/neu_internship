...
function OutGameData:initTower()
    ...
    unPacksOrdinary_ = { tower_20, }--{tower_1, tower_4, tower_7, tower_9, tower_18, tower_20, }
    packsOrdinary_[1]=PackItem.new(tower_1, 0)
    packsOrdinary_[2]=PackItem.new(tower_4, 0)
    packsOrdinary_[3]=PackItem.new(tower_7, 0)
    packsOrdinary_[4]=PackItem.new(tower_9, 0)
    packsOrdinary_[5]=PackItem.new(tower_18, 0)
...
--[[--
    ...
    @param tower,类型：table,塔数据
    @param i,类型：number,表下标

    @return none
]]
function OutGameData:initTowerByLevel(id,level)
    print(id,level)
    for i=1,level do
        self:getTower(id):atkUpgrade()
        self:getTower(id):fireCdUpgrade()
        self:getTower(id):valueUpgrade()
    end
end
...
function OutGameData:setCurrentTower(tower,i)
    currentTower[i]=tower
end
...
--[[--
    ...
    @return self.currentTower[i]，类型：table,塔数据
]]
function OutGameData:getCurrentTower(i)
    return currentTower[i]
end
...
function OutGameData:getTrophy()
    return self.trophy
end
--[[--
    修改奖杯的数据
...
function OutGameData:setTrophy(n)
    self.trophy = self.trophy + n
end
...
function OutGameData:goldShop()
    ...
    cc.UserDefault:getInstance():setIntegerForKey("a",a)
    cc.UserDefault:getInstance():setIntegerForKey("b",b)
    cc.UserDefault:getInstance():setIntegerForKey("c",c)
    cc.UserDefault:getInstance():setIntegerForKey("d",d)
    cc.UserDefault:getInstance():setIntegerForKey("e",e)
...
function OutGameData:oldGoldShop()
    local a=cc.UserDefault:getInstance():getIntegerForKey("a")
    local b=cc.UserDefault:getInstance():getIntegerForKey("b")
    local c=cc.UserDefault:getInstance():getIntegerForKey("c")
    local d=cc.UserDefault:getInstance():getIntegerForKey("d")
    local e=cc.UserDefault:getInstance():getIntegerForKey("e")
    local packs = {towersOrdinary_[a], towersOrdinary_[b], towersOrdinary_[c], towersRarity_[d], towersEpic_[e]}
    return packs
end
