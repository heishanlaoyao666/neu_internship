...
function MainView:onEnter()
    ...
    --匹配
    EventManager:regListener(EventDef.ID.FIGHT, self, function()
        self:performWithDelay(function()
            if cc.UserDefault:getInstance():getBoolForKey("fight") then
                scheduler.unscheduleGlobal(scheduler1)
                local AnotherScene=require("app.scenes.ingame.FightingScene"):new()
...
function MainView:onExit()
    ...
    EventManager:unRegListener(EventDef.ID.FIGHT, self)
end
