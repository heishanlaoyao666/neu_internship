...
function CurrentTowerLayer:initView()
    ...
    local data={lineup={},level={}}
    for i=1,3 do
        for j=1,5 do
            data.lineup[i*5-5+j]=cc.UserDefault:getInstance():getIntegerForKey("current"..j..i)
            data.level[i*5-5+j]=OutGameData:getTower(data.lineup[i*5-5+j]):getLevel()
        end
    end
    OutgameMsg:sendData(account,"lineupData",data)
