...
local data_ ={}  -- 类型： table ,接收数据的载体
local dataType_ -- 类型：string，接收数据的种类
-------------------------------------------------------------
-- 本地方法声明
-------------------------------------------------------------
local _init
...
function MsgController:returndata_()
    --print("returndata_="..data_["financeData"][1])
    return data_
end
-------------------------------------------------------------
-- 本地方法实现
...
function _handleMsg(event, data)
    ...
    elseif event == SimpleTCP.EVENT_DATA then
        ...
        if data then
            ...
            local data_str = json.decode(ba:readStringBytes(len))
            data_=data_str
            __handleMsgData(self, data_str)
...
--[[    local ba = ByteArray.new()
        ...
        ba:setPos(1)
        local len = ba:readInt()
        local data_S = json.decode(ba:readStringBytes(len))
        print(data_S)
...
function __handleMsgData(self, data)
    --print("data="..data["financeData"][1])
