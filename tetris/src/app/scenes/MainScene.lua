...
local Scene = require "app.scenes.BlockScene"
local Block = require "app.data.Block"
local NextBlock = require "app.data.NextBlock"
...
function MainScene:View()
    local img_score = ccui.Text:create("score:", "font/Marker Felt.ttf",24)
    img_score:pos(display.right * 1/6, display.top * 9.27/10)
    img_score:setAnchorPoint(0, 0.5)
    img_score:addTo(self)
    self.font_score = ccui.Text:create(score, "font/Marker Felt.ttf",24)
    self.font_score:pos(display.right * 2/6, display.top * 9.27/10)
