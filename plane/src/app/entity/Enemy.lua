...
function Enemy:ctor()
    ...
    local enemyFrameName = SystemConst.ENEMY_NAME
    local hitPointsTemp = SystemConst.ENEMY_INITIAL_HIT_POINTS
    local velocityTemp = SystemConst.ENEMY_VELOCITY
...
function Enemy:spawn()
    local yPos = self.size.height + self:getContentSize().height/2
    local xPos = math.random() * (self.size.width - self:getContentSize().width) + self:getContentSize().width/2
