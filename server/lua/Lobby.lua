...
local LobbyData = require("lua.outGame.LobbyData")
local StoreData = require("lua.outGame.StoreData")
...
function handleEndMapping(msg)
	LobbyData.mappingQueue:deleteItem(msg.pid)
end
...
function handleMapping()
	if LobbyData.mappingQueue:getQueueLength() <= 1 then
		return
	else -- 转发消息
		local playerOneMsg = nil
		local playerTwoMsg = nil

		while playerOneMsg == nil do
			if LobbyData.mappingQueue:getQueueLength() > 0 then
				playerOneMsg = LobbyData.mappingQueue:deQueue()
			else
				return
			end
		end

		while playerTwoMsg == nil do
			if LobbyData.mappingQueue:getQueueLength() > 0 then
				playerTwoMsg = LobbyData.mappingQueue:deQueue()
			else
				return
			end
		end
