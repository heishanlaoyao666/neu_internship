local Shezhi = class("Shezhi", function()
    return display.newScene("Shezhi")
end)
...
function Shezhi:ctor()
    ...
    local isMusic = SettingMusic:isMusic()
    self:createMiddleMiddlePanel()
    self:createMiddleBelowPanel()
    
end

function Shezhi:createMiddleMiddlePanel()
    ...
    new_gameButton:addTouchEventListener(function(sender, eventType)
		if 2 == eventType then
			local nextScene=import("app/scenes/SecScene"):new()
    ...
    local function onChangedCheckBox1(sender,eventType)
        ...
        if state then
            ...
            audio.playEffect("sounds/bgMusic.ogg")
            end)
        else 
            print("2")
...
function Shezhi:createMiddleBelowPanel()
    local width, height = display.width, display.top
    ...
    font:setAnchorPoint(0.5, 0.5)
    font:setTextColor(cc.c4b(255,255,255,128))
