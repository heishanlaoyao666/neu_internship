...
function BattleLayer:initView()
    ...
    --底图
    local basemapSprite = display.newSprite("artcontent/lobby(ongame)/battle_interface/basemap_interfaces.png")
    self.container_:addChild(basemapSprite)
    basemapSprite:setPosition(display.cx,display.cy)

    --对战模式按钮
    local battlemodeBtn = ccui.Button:create("artcontent/lobby(ongame)/battle_interface/button_battlemode.png")
    ...
    --已选队伍底图
    local teamsSprite = display.newSprite("artcontent/lobby(ongame)/battle_interface/basemap_selectedteams.png")
    ...
    --天梯背景和ListView创建
    local rankSprite = display.newSprite("artcontent/lobby(ongame)/battle_interface/rank/basemap_rank.png")
    ...
    local slideSprite = display.newSprite("artcontent/lobby(ongame)/battle_interface/rank/icon_slide.png")
    ...
    local slidesignSprite = display.newSprite("artcontent/lobby(ongame)/battle_interface/rank/icon_slidesign.png")
    ...
    for i=1,22 do
        if cc.UserDefault:getInstance():getIntegerForKey("imgstatus"..i) then
            imgStatus[i]=cc.UserDefault:getInstance():getIntegerForKey("imgstatus"..i)
        else
            imgStatus[i]=1
        end
    end
    for i=1,22 do
        if i<=4 then
            if i*50<=cc.UserDefault:getInstance():getIntegerForKey("奖杯数") and imgStatus[i]==1 then
                imgStatus[i]=2
            end
        end
        if i>=5 then
            if (i*100-200)<=cc.UserDefault:getInstance():getIntegerForKey("奖杯数") and imgStatus[i]==1 then
                imgStatus[i]=2
            end
        end
        gold[i]=0
        ...
        --宝箱内容
        if i==1 then
            local content = display.newSprite("artcontent/lobby(ongame)/store/diamondstore/chest_legend.png")
            img:addChild(content)
            content:setScale(0.6)
            content:setAnchorPoint(0.5, 0.5)
            content:setPosition(img:getContentSize().width/2,img:getContentSize().height/2)
        elseif i==2 then
            content = display.newSprite("artcontent/lobby(ongame)/battle_interface/rank/gold.png")
            img:addChild(content)
            content:setAnchorPoint(0.5, 0.5)
            ...
            gold[i]=500
        elseif i<12 then
            content = display.newSprite("artcontent/lobby(ongame)/store/diamondstore/chest_epic.png")
            img:addChild(content)
            content:setScale(0.6)
            content:setAnchorPoint(0.5, 0.5)
            content:setPosition(img:getContentSize().width/2,img:getContentSize().height/2)
        elseif i==12 then
            content = display.newSprite("artcontent/lobby(ongame)/store/goldstore/itemicon_tower/13.png")
            img:addChild(content)
            content:setScale(0.5)
            content:setAnchorPoint(0.5, 0.5)
            content:setPosition(img:getContentSize().width/2,img:getContentSize().height/2)
        elseif i==22 then
            content = display.newSprite("artcontent/lobby(ongame)/battle_interface/rank/group92.png")
            img:addChild(content)
            content:setAnchorPoint(0.5, 0.5)
            content:setPosition(img:getContentSize().width/2,img:getContentSize().height/2)
        elseif i%3==1 then
            ...
            content:setPosition(img:getContentSize().width/2,img:getContentSize().height/2)
        elseif i%3==2 then
            content = display.newSprite("artcontent/lobby(ongame)/battle_interface/rank/diamonds.png")
            img:addChild(content)
            content:setAnchorPoint(0.5, 0.5)
            content:setPosition(img:getContentSize().width/2,img:getContentSize().height/2+20)
            display.newTTFLabel({
        ...
        local lockedSprite = display.newSprite("artcontent/lobby(ongame)/battle_interface/rank/locked.png")
        ...
        local getSprite = display.newSprite("artcontent/lobby(ongame)/battle_interface/rank/group130.png")
        ...
        if imgStatus[i]==1 then
        elseif imgStatus[i]==2 then
