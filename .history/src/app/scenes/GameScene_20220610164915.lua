...
function GameScene:init()
    ...
    if GameHandler.isContinue == true then
        ...
        if GameHandler.BulletData ~= nil and GameHandler.BulletData ~= {} and #(GameHandler.BulletData) > 0 then
            for i = 1, #(GameHandler.BulletData) do
                local bullet = BulletNode:create(0, 0, GameHandler.BulletData[i]):addTo(self)
            end
        end
        Log.i(tostring(#(GameHandler.EnemyData)))
        if GameHandler.EnemyData ~= nil and GameHandler.EnemyData ~= {} and #(GameHandler.EnemyData) > 0 then
    ...
    GameHandler.updateContinue(false)
end
