...
function GameScene:onEnter()
    ...
    --- 我方飞机
    self.myRoleWithTail =
        myRoleNode:create(ConstantsUtil.ROLE_PURPLE_PLANE, 2, WinSize.width * 0.5, WinSize.height * 0.17):addTo(self)
    ...
    local hp_item =
        ...
        TypeConvert.Integer2StringLeadingZero(self.myRoleWithTail.dataModel.hp, 3),
    ...
    local score_item =
        ccui.TextBMFont:create(
        TypeConvert.Integer2StringLeadingZero(self.myRoleWithTail.dataModel.score, 3),
        ConstantsUtil.PATH_BIG_NUM
    )
    ...
    local function addBullet()
        ...
        local function bulletMove()
            ...
            if bulletY > WinSize.height then
                ...
                table.removebyvalue(ConstantsUtil.bulletArray, bullet, false)
            end
        end
        -- 每帧刷新一次
    ...
    local function newEnemy()
        ...
        local function enemyMove()
            ...
            if newEnemyY <= ConstantsUtil.DIE_PLACE_ENEMY then
                ...
                table.removebyvalue(ConstantsUtil.enemyArray, enemy, false)
            end
        end
        enemy:scheduleUpdateWithPriorityLua(enemyMove, 0)
        table.insert(ConstantsUtil.enemyArray, enemy)
    end
    ...
    local function collisionBetweenBUlletAndEnemy()
        local bulletArraySize = #(ConstantsUtil.bulletArray)
        local enemyArraySize = #(ConstantsUtil.enemyArray)
        for i = 1, bulletArraySize do
            if #(ConstantsUtil.bulletArray) < i then
                break
            end
            local rectA = ConstantsUtil.bulletArray[i]:getBoundingBox()
            for j = 1, enemyArraySize do
                if #(ConstantsUtil.enemyArray) < j then
                    break
                end
                local rectB = ConstantsUtil.enemyArray[j]:getBoundingBox()
                if
                    math.abs(ConstantsUtil.bulletArray[i]:getPositionX() - ConstantsUtil.enemyArray[j]:getPositionX()) *
                        2 <=
                        (rectA.width + rectB.width) and
                        (math.abs(
                            ConstantsUtil.bulletArray[i]:getPositionY() - ConstantsUtil.enemyArray[j]:getPositionY()
                        ) *
                            2) <=
                 ...
                 then
                    ...
                    -- score
                    if self.myRoleWithTail.dataModel.score < 999 then
                        self.myRoleWithTail.dataModel.score =
                            self.myRoleWithTail.dataModel.score + ConstantsUtil.PLUS_ENEMY_SCORE
                        score_item:setString(
                            TypeConvert.Integer2StringLeadingZero(self.myRoleWithTail.dataModel.score, 3)
                        )
                    end
                    -- 爆炸动画
                    getExplosion(ConstantsUtil.enemyArray[j]:getPositionX(), ConstantsUtil.enemyArray[j]:getPositionY())
                    -- body
                    ConstantsUtil.bulletArray[i]:removeFromParent()
                    ConstantsUtil.enemyArray[j]:removeFromParent()
                    table.remove(ConstantsUtil.bulletArray, i)
                    table.remove(ConstantsUtil.enemyArray, j)
                    i = i - 1
                    j = j - 1
    ...
    local function collisionBetweenMyRoleAndEnemy()
        ...
        for i = 1, enemyArraySize do
            if #(ConstantsUtil.enemyArray) < i then
                break
            end
            local rect = ConstantsUtil.enemyArray[i]:getBoundingBox()
            if
                math.abs((ConstantsUtil.enemyArray[i]:getPositionX() - myRole:getPositionX())) * 2 <=
                    (rect.width + rectMyRole.width) and
                    math.abs(ConstantsUtil.enemyArray[i]:getPositionY() - myRole:getPositionY()) * 2 <=
             ...
             then
                ...
                --- 爆炸动画
                if self.myRoleWithTail.dataModel.hp - ConstantsUtil.MINUS_ENEMY_COLLISION > 0 then
                    self.myRoleWithTail.dataModel.hp =
                        self.myRoleWithTail.dataModel.hp - ConstantsUtil.MINUS_ENEMY_COLLISION
                    hp_item:setString(TypeConvert.Integer2StringLeadingZero(self.myRoleWithTail.dataModel.hp, 3))
                else
                    ConstantsUtil.puase = true
                ...
                --body
                ConstantsUtil.enemyArray[i]:removeFromParent()
                table.remove(ConstantsUtil.enemyArray, i)
                i = i - 1
                enemyArraySize = enemyArraySize - 1
            end
        end
