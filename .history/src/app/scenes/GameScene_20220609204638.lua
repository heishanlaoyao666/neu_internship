...
function GameScene:init()
    if GameHandler.myRole == nil then
        GameHandler.myRole = MyRoleNode:create(ConstantsUtil.ROLE_PURPLE_PLANE)
    end
end
...
function GameScene:onEnter()
        ...
        function(ref, event)
            ...
            elseif cc.EventCode.ENDED == event then
                ...
                if GameHandler.pause == false then
                    ...
                    GameHandler.PlaneData = clone(GameHandler.myRole)
                    --
    ...
    local function collisionBetweenMyRoleAndEnemy()
        ...
        for i = 1, enemyArraySize do
            if #(GameHandler.EnemyArray) < i or GameHandler.EnemyArray[i] == nil then
                break
            end
            if GameHandler.EnemyArray[i]:isCollision(GameHandler.myRole) then
                ...
                if GameHandler.myRole:getMyHp() - ConstantsUtil.MINUS_ENEMY_COLLISION > 0 then
                    ...
                    GameHandler.pause = true
                    Director:pause()
                    local over = OverNode:create(ConstantsUtil.COLOR_GREW_TRANSLUCENT, GameHandler.myRole)
                    over:addTo(self)
                end
