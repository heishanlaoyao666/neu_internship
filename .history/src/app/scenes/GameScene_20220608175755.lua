...
function GameScene:onEnter()
    ...
    local function collisionBetweenBUlletAndEnemy()
        ...
        local ConstantsUtil.enemyArraySize = #(ConstantsUtil.enemyArray)
        for i = 1, bulletArraySize do
            if #(ConstantsUtil.bulletArray) < i then
                break
            end
            local rectA = ConstantsUtil.bulletArray[i]:getBoundingBox()
            for j = 1, ConstantsUtil.enemyArraySize do
                if #(ConstantsUtil.enemyArray) < j then
                    break
                end
                local rectB = ConstantsUtil.enemyArray[j]:getBoundingBox()
                if
                    math.abs(ConstantsUtil.bulletArray[i]:getPositionX() - ConstantsUtil.enemyArray[j]:getPositionX()) * 2 <=
                        (rectA.width + rectB.width) and
                        (math.abs(ConstantsUtil.bulletArray[i]:getPositionY() - ConstantsUtil.enemyArray[j]:getPositionY()) * 2) <=
                 ...
                 then
                    ...
                    ConstantsUtil.enemyArraySize = ConstantsUtil.enemyArraySize - 1
                end
            end
        end
    end
    ...
    local function collisionBetweenMyRoleAndEnemy()
        ...
        for i = 1, ConstantsUtil.enemyArraySize do
            if #(ConstantsUtil.enemyArray) < i then
                break
            end
            local rect = ConstantsUtil.enemyArray[i]:getBoundingBox()
            if
                math.abs((ConstantsUtil.enemyArray[i]:getPositionX() - myRole:getPositionX())) * 2 <= (rect.width + rectMyRole.width) and
