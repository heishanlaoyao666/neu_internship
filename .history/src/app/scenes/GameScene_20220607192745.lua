...
function GameScene:onEnter()
    ...
    local function collisionBetweenBUlletAndEnemy()
        ...
        for i = 1, bulletArraySize do
            ...
            local rectA = bulletArray[i].bullet:getBoundingBox()
            for j = 1, enemyArraySize do
                if #(enemyArray) < j then
                    break
                end
                local rectB = enemyArray[j].enemy:getBoundingBox()
                if
                    math.abs(bulletArray[i].bullet:getPositionX() - enemyArray[j].enemy:getPositionX()) * 2 <=
                        (rectA.width + rectB.width) and
                        (math.abs(bulletArray[i].bullet:getPositionY() - enemyArray[j].enemy:getPositionY()) * 2) <=
                 ...
                 then
                    ...
                    -- 爆炸动画
                    getExplosion(enemyArray[j].enemy:getPositionX(), enemyArray[j].enemy:getPositionY())
                    -- body
    ...
    local function collisionBetweenMyRoleAndEnemy()
        ...
        for i = 1, enemyArraySize do
            ...
            local rect = enemyArray[i].enemy:getBoundingBox()
            if
                math.abs((enemyArray[i].enemy:getPositionX() - myRole:getPositionX())) * 2 <=
                    (rect.width + rectMyRole.width) and
                    math.abs(enemyArray[i].enemy:getPositionY() - myRole:getPositionY()) * 2 <=
