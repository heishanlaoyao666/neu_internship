...
function RankNode:create(rankPos, nickname, score, rankData)
    -- body
    local rank = RankNode.new(nickname, score, rankData)
    rank:addChild(rank:init(rankPos))
    return rank
end

function RankNode:ctor(nickname, score, rankData)
...
function RankNode:init(rank)
    ...
    local itemRank = ccui.TextBMFont:create(tostring(rank), ConstantsUtil.PATH_BIG_NUM)
