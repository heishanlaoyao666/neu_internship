...
function PauseNode:create(itsColor, myRoleSave)
    -- body
    local pause = PauseNode.new()
    pause:addChild(pause:init(itsColor, myRoleSave))
    return pause
end
...
function PauseNode:init(itsColor, myRoleSave)
    -- body
    ...
    backButton:addTouchEventListener(
        function(ref, event)
            ...
            elseif cc.EventCode.ENDED == event then
                ...
                -- Log.i(tostring(lastGameScene ~= nil))
                Director:resume()
                ConstantsUtil.myRoleSave = clone(myRoleSave)
