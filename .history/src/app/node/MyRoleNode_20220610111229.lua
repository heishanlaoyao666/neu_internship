...
-- TODO涂装 如果后续扩展涂装 那么这里需要进行修改
function MyRoleNode:create(painting, myPlaneData)
    local myRoleNode = MyRoleNode.new(myPlaneData)
    myRoleNode:addChild(myRoleNode:init(painting))
    return myRoleNode
end

function MyRoleNode:ctor(myPlaneData)
    -- 这里的DataModel只考虑了飞机，没有管飞机尾巴，可能在后续扩展会有影响
    if myPlaneData == nil then
        self.dataModel = PlaneModel.new()
    else
        self.dataModel = myPlaneData
    end
    -- plane
    self.tailFlame = nil
    self.myRole = nil
...
function MyRoleNode:getPosition()
    ...
    return self.myRole:getPositionX(), self.myRole:getPositionY()
end
...
function MyRoleNode:getContentSize()
    return self.myRole:getContentSize()
end
