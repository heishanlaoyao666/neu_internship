...
function EnemyNode:init(x, y)
    ...
    local function move()
        ...
        if self.y <= ConstantsUtil.DIE_PLACE_ENEMY then
            ...
            table.removebyvalue(enemyArray, self, false)
        end
    end
    self.enemy:scheduleUpdateWithPriorityLua(move, ConstantsUtil.LEVEL_VISIABLE_MEDIUM)
    table.insert(enemyArray, self)
    return layer
end
