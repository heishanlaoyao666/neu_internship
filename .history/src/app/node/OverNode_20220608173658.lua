...
function OverNode:init(itsColor, myRoleWithTail)
    ...
    local viewWidth = WinSize.width / 2
    local viewHeight = WinSize.height / 2
    listView:setPosition(viewWidth, viewHeight)
    ...
    infoLayer:setContentSize()
    infoLayer:setDirection(ccui.HORIZONTAL)
    local nicknameField = ccui.Layout():create():addTo(infoLayer)
    local nickname = ccui.Text:create(ConstantsUtil.username, ConstantsUtil.PATH_NORMAL_FONT_TTF):addTo(nicknameField)
