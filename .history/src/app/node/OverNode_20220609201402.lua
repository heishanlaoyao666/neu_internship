...
function OverNode:init(itsColor, myRole)
    ...
    local restartButton =
        ccui.Button:create(ConstantsUtil.PATH_OVER_RESTART_PNG):addTo(
        listView,
    ...
    restartButton:addTouchEventListener(
        function(ref, event)
            ...
            elseif cc.EventCode.ENDED == event then
                ...
                --
                ConstantsUtil.nowScene = import("app.scenes.GameScene").new()
                display.replaceScene(ConstantsUtil.nowScene)
            end
        end
    )
